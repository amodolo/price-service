...
class DiscountCalculator {
    float calculateFinalPrice(@NotNull Product product, int quantity, ValueDiscount discount) {
    ...
    float calculateFinalPrice(@NotNull Product product, int quantity, PercentageDiscount discount) {
        Objects.requireNonNull(product);
        var price = product.getPrice() * quantity;
